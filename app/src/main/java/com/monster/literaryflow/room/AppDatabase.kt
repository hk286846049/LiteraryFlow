...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // 在现有表上添加新的列
                database.execSQL("ALTER TABLE auto_info ADD COLUMN loop_type TEXT NOT NULL DEFAULT 'DAY_LOOP'")
                database.execSQL("ALTER TABLE auto_info ADD COLUMN week_data TEXT NOT NULL DEFAULT '[1]'")
                database.execSQL("ALTER TABLE auto_info ADD COLUMN sleep_time INTEGER NOT NULL DEFAULT 0")
            }
        }
